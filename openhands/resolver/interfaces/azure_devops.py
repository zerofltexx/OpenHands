...
from openhands.resolver.interfaces.issue import Issue, IssueHandlerInterface, ReviewThread
...
class AzureDevOpsIssueHandler(IssueHandlerInterface):
    ...
    def set_owner(self, owner: str) -> None:
        ...
        if len(parts) >= 2:
            ...
            self.base_api_url = f"https://dev.azure.com/{self.organization}/{self.project}/_apis"
            self.repo_api_url = f"{self.base_api_url}/git/repositories/{self.repository}"
            self.work_items_api_url = f"{self.base_api_url}/wit"
    ...
    def download_issues(self) -> list[Any]:
        ...
        response = httpx.post(
            wiql_url, headers=self.get_headers(), json=wiql_query
        )
    ...
    def request_reviewers(self, reviewer: str, pr_number: int) -> None:
        ...
        for user in users:
            if user.get("displayName") == reviewer or user.get("mailAddress") == reviewer:
    ...
    def get_context_from_external_issues_references(
        ...
        if review_threads:
            for thread in review_threads:
                context.append(f"Review thread for files {', '.join(thread.files)}:\n{thread.comment}")
    ...
    def get_converted_issues(
        ...
        for work_item in work_items:
            ...
            for relation in work_item.get("relations", []):
                if relation.get("rel") == "ArtifactLink" and "pullrequest" in relation.get("url", ""):
                    is_pr = True
            ...
            if is_pr and pr_number:
                ...
                head_branch = pr_data.get("sourceRefName", "").replace("refs/heads/", "")
                base_branch = pr_data.get("targetRefName", "").replace("refs/heads/", "")
                ...
                for thread in threads:
                    thread_comments = [comment.get("content", "") for comment in thread.get("comments", [])]
                    review_comments.extend(thread_comments)
                    
                    # Get files associated with this thread
                    thread_files = []
                    if thread.get("threadContext", {}).get("filePath"):
                        thread_files.append(thread.get("threadContext", {}).get("filePath"))
