...
async def test_azure_devops_service_init():
    ...
    service = AzureDevOpsService(
        user_id="test_user",
        token=None,
        base_domain="myorg/myproject",
    )
    
    assert service.organization == "myorg"
    assert service.project == "myproject"
...
async def test_azure_devops_get_repositories():
    ...
    with patch("httpx.AsyncClient") as mock_client:
        ...
        assert repos[0].full_name == "myorg/Project1/Repo1"
        assert repos[0].git_provider == ProviderType.AZURE_DEVOPS
        assert repos[1].id == "repo2"
        assert repos[1].full_name == "myorg/Project1/Repo2"
...
async def test_azure_devops_get_repository_details():
    ...
    with patch("httpx.AsyncClient") as mock_client:
        ...
        # Call the method
        repo = await service.get_repository_details_from_repo_name("myorg/Project1/Repo1")
